...
```{r}
renderText({ 
    scatter_data_all <- df %>% select(iso3c, country, date, unname(titles_indicators))
...
renderPlotly({
    ...
    scatter_data <- df %>% select(iso3c, country, date, unname(titles_indicators)) %>% filter(date==input$dateSel)
    title_scatter <- paste(names(titles_indicators)[titles_indicators == input$Indicator1], 
...
# 
# 
# ggplotly(ggplot(dat3,aes(x=Category,y=Quantity))+
#   geom_point(aes(frame = Time), color="black")+
#   geom_segment(aes(x=Category,xend=Category,y=0,yend=Quantity, frame = Time), color="black")+

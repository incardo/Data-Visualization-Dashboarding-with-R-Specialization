...
ggplot(data = map_data_combined, mapping = aes(x= long, y= lat, group = group,fill=Score))+geom_polygon(color="black")+
  scale_fill_distiller(palette=5)
...
mexico <- filter(world,region %in% c("Costa Rica", "Panama", "Nicaragua"))

my_cities <-maps::world.cities
big_cities <- filter(my_cities,country.etc %in% c("Costa Rica", "Panama", "Nicaragua") & pop>40000) %>%
              add_column(Measurement)

ggplot(data = mexico, mapping = aes(x= long, y= lat, group = group))+
  geom_polygon(color="black",fill="white")+
  geom_point(data = big_cities,aes(x=long,y=lat,group=NULL,color=Measurement,size=5))+

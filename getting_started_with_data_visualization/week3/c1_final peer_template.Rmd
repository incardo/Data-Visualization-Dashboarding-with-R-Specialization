...
filtered_df <- dat %>% filter(gender==1 & region %in% c(1,2) & marstat == 1 & newsint ==1)
str(filtered_df)
```
...
new_data <- dat %>% mutate(tax_scale= recode(CC18_325d,`1`=1,`2`=0)+recode(CC18_325a,`1`=1,`2`=0))
new_data$tax_scale[1:20]
...
```{r problem10}
dat %>% filter(investor == 2 & faminc_new %in% c(5:10)) %>% summarise(`Mean Religion Imp.` = mean(pew_religimp), 
...
```{r problem11}
kable(summarise(dat, mean(faminc_new),median(faminc_new),sd(faminc_new)))
